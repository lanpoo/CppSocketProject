...
void bootUp(int argc, char* argv[]){
    ...
    cout << "Hospital C is up and running using UDP on port 30249.\n";
    locaHosp = atoi(argv[1]);
    totalCapacity = static_cast<double>(atof(argv[2]));
    initOccupancy = static_cast<double>(atof(argv[3]));
    cout << "Hospital C has total capacity "<< totalCapacity << " and initial occupancy "<< initOccupancy << ".\n";
    socklen_t len = 0;
...
void recvMessage(){
    ...
    recvbuf[recvnum] = '\0';
    cout << "Hospital C has received input from client at location " << recvbuf << endl;
    ...
    if(nodeInfo.find(location) == nodeInfo.end()){
        disValid = false;
        cout << "Hospital C does not have the location " << location << " in map\n";
    }else{
        if(location == locaHosp){
            ...
            cout << "Hospital C has capacity = " << totalCapacity << ",occupation= " 
                 <<initOccupancy << ", availability =" << availability << endl;
            cout << "Hospital C has found the shortest path to client,distance = ";
            if(distance == 0){
                disValid = false;
                cout << "None" << endl;
            }else{
    ...
    if(!disValid || !avaValid){
        score = "None+None";
        cout << "Hospital C has the score = None\n";
        sendToSched(score);
        ...
        sendToSched(score);
        cout << "Hospital C has sent score = "<< score_ << " and distance = " << distance << "  to the Scheduler" <<endl;
    }
