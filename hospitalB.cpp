...
void bootUp(int argc, char* argv[]){
    ...
    cout << "Hospital B is up and running using UDP on port 31249.\n";
    locaHosp = atoi(argv[1]);
    totalCapacity = static_cast<double>(atof(argv[2]));
    initOccupancy = static_cast<double>(atof(argv[3]));
    cout << "Hospital B has total capacity "<< totalCapacity << " and initial occupancy "<< initOccupancy << ".\n";
    socklen_t len = 0;
...
void recvMessage(){
    ...
    recvbuf[recvnum] = '\0';
    cout << "Hospital B has received input from client at location " << recvbuf << endl;
    ...
    if(nodeInfo.find(location) == nodeInfo.end()){
        disValid = false;
        cout << "Hospital B does not have the location " << location << " in map\n";
    }else{
        if(location == locaHosp){
            ...
            cout << "Hospital B has capacity = " << totalCapacity << ",occupation= " 
                 <<initOccupancy << ", availability =" << availability << endl;
            cout << "Hospital B has found the shortest path to client,distance = ";
            if(distance == 0){
                disValid = false;
                cout << "None" << endl;
            }else{
    ...
    if(!disValid || !avaValid){
        score = "None+None";
        cout << "Hospital B has the score = None\n";
        sendToSched(score);
        ...
        sendToSched(score);
        cout << "Hospital B has sent score = "<< score_ << " and distance = " << distance << "  to the Scheduler" <<endl;
    }
